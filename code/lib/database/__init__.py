...
def create_users() -> None:
    ...
    try:
        ...
        type VARCHAR(9) CHECK (type IN ('student', 'professor', 'admin')) NOT NULL
        );
        """)
        cursor.execute(query)
        con.commit()
    except:
        print('\033[0;31mERRO: Falha ao criar a tabela users!\033[0m')
    else:
        print('\033[0;32mTabela "users" criada com sucesso!\033[0m')
...
def add_student(name: str, sex: str, birth: date, cpf: str, email: str, edu: str, grade: str, table: str) -> None:
    ...
    try:
        ...
        query = sql.SQL("""INSERT INTO {} 
            (nome, sexo, nascimento, cpf, email, ensino, serie) 
            VALUES 
            (%s, %s, %s, %s, %s, %s, %s);""").format(sql.Identifier(table))
...
def update_student(id: int, value, column: str, table: str) -> None:
    ...
    """).format(sql.Identifier(table), sql.Identifier(column))
        ...
        print(f'\033[0;31mERRO: Falha ao atualizar aluno com id {id}!\033[0m')
    else:
